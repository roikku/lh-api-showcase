...
public class NearestAirportsPresenter extends BasicAbstractFormResultPresenter {

	private Logger logger = Logger.getLogger("NearestAirportPresenter");
	...
	public NearestAirportsPresenter (ReferenceDataServiceAsync referenceDataService, HandlerManager eventBus, Display view) {
		super (eventBus, view) ;
		...
		if (display.getGoHandler() != null) {
			display.getGoHandler().addClickHandler(new ClickHandler() {
				public void onClick(ClickEvent event) {
					...
					fetchNearestAirportsData (display.getLatitude(), display.getLongitude(), display.getLang()) ;
				}
			});
		}
	}
	...
	private void fetchNearestAirportsData (Double latitude, Double longitude, LanguageCode languageCode) {
		
		display.setBusy(true);
		referenceDataService.getNearestAirports(latitude, longitude, languageCode, new AsyncCallback<String>() {
			@Override
			...
			public void onSuccess(final String result) {
				display.setBusy(false);
				display.setJson(result, new NearestAirportsResponseJsonParser());
			}
		});
	}
}

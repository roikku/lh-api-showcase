...
public class CountriesPresenter extends BasicAbstractFormResultPresenter {

	private Logger logger = Logger.getLogger("CountriesPresenter");
	...
	public CountriesPresenter (ReferenceDataServiceAsync referenceDataService, HandlerManager eventBus, Display view) {
		super (eventBus, view) ;
		...
		if (display.getGoHandler() != null) {
			display.getGoHandler().addClickHandler(new ClickHandler() {
				public void onClick(ClickEvent event) {
					...
					fetchCountriesData (display.getCountryCode(), display.getLang()) ;
				}
			});
		}
	}
	...
	private void fetchCountriesData (CountryCode countryCode, LanguageCode lang) {
		
		display.setBusy(true);
		referenceDataService.getCountries(countryCode, lang, new AsyncCallback<String>() {
					@Override
					...
					public void onSuccess(final String result) {
						display.setBusy(false);
						display.setJson(result, new CountriesResponseJsonParser ());
					}
				});
	}
}

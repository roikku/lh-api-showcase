...
public class CitiesPresenter extends BasicAbstractFormResultPresenter {

	private Logger logger = Logger.getLogger("CitiesPresenter");
	...
	public CitiesPresenter (ReferenceDataServiceAsync referenceDataService, HandlerManager eventBus, Display view) {
		super (eventBus, view) ;
		...
		if (display.getGoHandler() != null) {
			display.getGoHandler().addClickHandler(new ClickHandler() {
				public void onClick(ClickEvent event) {
					...
					fetchCitiesData (display.getCityCode(), display.getLang()) ;
				}
			});
		}
	}
	...
	private void fetchCitiesData (CityCode cityCode, LanguageCode languageCode) {
		
		display.setBusy(true);
		referenceDataService.getCities(cityCode, languageCode, new AsyncCallback<String>() {
			@Override
			...
			public void onSuccess(final String result) {
				display.setBusy(false);
				display.setJson(result, new CitiesResponseJsonParser());
			}
		});
	}

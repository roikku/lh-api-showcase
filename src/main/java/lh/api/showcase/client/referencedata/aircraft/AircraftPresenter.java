...
public class AircraftPresenter extends BasicAbstractFormResultPresenter {
	...
	public AircraftPresenter (ReferenceDataServiceAsync referenceDataService, HandlerManager eventBus, Display view) {
		super (eventBus, view) ;
		...
		if (display.getGoHandler() != null) {
			display.getGoHandler().addClickHandler(new ClickHandler() {
				public void onClick(ClickEvent event) {
					...
					fetchAircraftData (display.getAircraftCode()) ;
				}
			});
		}
	}
	...
	private void fetchAircraftData (AircraftCode aircraftCode) {
		
		display.setBusy(true);
		referenceDataService.getAircraft(aircraftCode, new AsyncCallback<String>() {
			@Override
			...
			public void onSuccess(final String result) {
				display.setBusy(false);
				display.setJson(result, new AircraftResponseJsonParser());
			}
		});
	}
}
